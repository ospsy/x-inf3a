...
namespace Imagine {
	...
	//use planarThreshold to reject planar degenerate configuration
	//to reject if planar at k pixel -> then put	planarThreshold=k*k; 
	//if not to be used let default: ->				planarThreshold=0.0;
		...
		double planarThreshold=0.0)
	{
		...
		for (int i=0; i < N; i++) {
			...
			estimator(sample, std::back_inserter(paramVec),planarThreshold);
