...
public class PolyhedronBoxTree {
	...
	int cutDim ;
	double cutValue ;
	Double distanceMin ; // Utile pour le calcul de distance à un point : à chaque instant, on sait qu'elle est < distanceMin
	...
	public void computeTree ()
	{
		...
		it = facets.iterator() ;
		boolean lastLeft = (Math.random() < 0.5) ; // Pour la répartition des faces à cheval
		while (it.hasNext())
		{
			Face<Point_3> f = it.next() ;
