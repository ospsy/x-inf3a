...
public class Taubin extends CourbureEstimator {
	...
	protected void computeCurvatureAtVertex(Vertex<Point_3> v) {
		...
		// Récupération des projections des voisins sur le plan tangent
		...
		for (Vertex<Point_3> vertex : voisins) {
			...
			Matrix mNormal = new Matrix (3,1) ;
			mNormal.set(0, 0, normal.getX().doubleValue()) ;
			mNormal.set(1, 0, normal.getY().doubleValue()) ;
			mNormal.set(2, 0, normal.getZ().doubleValue()) ;
			...
			m = m.plus(Tij.times(Tij.transpose())).times(wij*kij/w) ;
