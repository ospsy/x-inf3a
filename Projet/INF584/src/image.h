...
public:
	...
	inline void set(int x, int y, const Vec3Df& color){
		if(color[0]<0)
			...
			data[3*(y+x*sizeY)]=255;
		else data[3*(y+x*sizeY)]=(unsigned char)(color[0]);

		if(color[1]<0)
			data[3*(y+x*sizeY)+1]=0;
		else if(color[1]>255)
			data[3*(y+x*sizeY)+1]=255;
		else data[3*(y+x*sizeY)+1]=(unsigned char)(color[1]);

		if(color[2]<0)
			data[3*(y+x*sizeY)+2]=0;
		else if(color[2]>255)
			data[3*(y+x*sizeY)+2]=255;
		else data[3*(y+x*sizeY)+2]=(unsigned char)(color[2]);
