...
void pbGPU(IplImage *im, IplImage* grad, IplImage* ori, bool gPb){
  ...
  for(int i=1;i<=3;i++){
    cvSetImageCOI(im,i);
    cvSetImageCOI(im2,4-i);//BGR -> RGB conversion
    cvCopy(im,im2);
  }
  cvSetImageCOI(im2,0);
  cvSetImageCOI(im,0);
  ...
  if(cvGetSize(im).width>maxWidth){
    int newHeight=(int)(cvGetSize(im).height*800/(double)(cvGetSize(im).width));
    IplImage* im3=cvCreateImage( cvSize(maxWidth,newHeight), IPL_DEPTH_8U, 4 );
    IplImage* gradTmp=cvCreateImage( cvSize(maxWidth,newHeight), IPL_DEPTH_32F, 1 );
    IplImage* oriTmp=cvCreateImage( cvSize(maxWidth,newHeight), IPL_DEPTH_32F, 1 );
    cvResize(im2,im3);
    damascene(maxWidth,newHeight,(uint*)(im3->imageData),(float*)(gradTmp->imageData),(float*)(ori->imageData),gPb);
    cvResize(gradTmp,grad);
    cvResize(oriTmp,ori);
  }else{
