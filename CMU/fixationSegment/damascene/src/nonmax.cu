...
void nonMaxSuppression(int p_nWidth, int p_nHeight, float* p_devPB, int p_nDevPBPitch, float* devNMax, int* devOri)
{
	...
	CUDA_SAFE_CALL(cudaMalloc((void**)&devMaxPB, size * sizeof(float)));
	//gettimeofday(&start, 0);
	...
	CUDA_SAFE_CALL(cudaFree(devMaxPB));
