...
//3)
int segLayer::readPbBoundary(char *prefixTag){
	char tmpName[100];
	sprintf(tmpName,"%s_pbBoundary.png",prefixTag);
	IplImage *tmp=cvLoadImage(tmpName,CV_LOAD_IMAGE_GRAYSCALE);
	if(!tmp){
		fprintf(stderr,"No pbBoundary file : %s\n",tmpName);
		exit(1);
	}else if(tmp->width!=width || tmp->height!=height){
		fprintf(stderr,"PbBoundary file doesn't have the same size as resized img\n");
		exit(1);
	}
	pbBoundary = cvCreateImage(cvSize(width,height),IPL_DEPTH_32F,1);
	cvScale(tmp, pbBoundary, 1.0/255);
	cvReleaseImage(&tmp);
	
	sprintf(tmpName,"%s_orients.png",prefixTag);
	tmp=cvLoadImage(tmpName,CV_LOAD_IMAGE_GRAYSCALE);
	if(!tmp){
		fprintf(stderr,"No orients file : %s\n",tmpName);
		exit(1);
	}else if(tmp->width!=width || tmp->height!=height){
		fprintf(stderr,"Orients file doesn't have the same size as resized img\n");
		exit(1);
	}
	edgeOri = cvCreateImage(cvSize(width,height),IPL_DEPTH_32F,1);
	cvScale(tmp, edgeOri, M_PI/255);
	cvReleaseImage(&tmp);
}
...
//3)
int segLayer::readFixPts(char *fixTxtFile,float scale){
	std::cout << fixTxtFile;
	std::ifstream file(fixTxtFile, std::ifstream::in);
        if(file) 
        {
		char l[60];
		while(file.getline(l,60)){
			float x=0,y=0;
			sscanf(l,"%f %f",&x,&y);
			assignFixPt((int)(x*scale),(int)(y*scale));
		}
                file.close();
        }
        else
                std::cout << "Unable to open file : " << fixTxtFile << std::endl;
        return 0;
}
...
int segLayer::saveResizedRegions(char* prefixTag, int width, int height){
	IplImage* tmp = cvCloneImage(img);//cvCreateImage(cvSize(this->width,this->height), IPL_DEPTH_8U, 1);
	IplImage* tmp2 = cvCreateImage(cvSize(width,height), IPL_DEPTH_8U, 3);//cvCreateImage(cvSize(width,height), IPL_DEPTH_8U, 1);
	char segName[100];
     int numContours = allContours.size();
	for(int i=0; i<numContours; i++){
		/*cvDrawContours(
...
int segLayer::savePbBoundary(char *prefixTag){
  ...
  IplImage* tmpOrient = cvCreateImage(cvGetSize(pbBoundary), IPL_DEPTH_8U, 1);
  cvScale(edgeOri, tmpOrient, 255./M_PI);	
  if (prefixTag == NULL){
    char pbMapName_noprefix[100];
    sprintf(pbMapName_noprefix,"pbBoundary.png");
    cvSaveImage(pbMapName_noprefix, tmpImg);
    sprintf(pbMapName_noprefix,"orients.png");
    cvSaveImage(pbMapName_noprefix, tmpOrient);
  }
  else{
    char pbMapName[100];
    sprintf(pbMapName,"%s_pbBoundary.png",prefixTag);
    cvSaveImage(pbMapName, tmpImg);  
    sprintf(pbMapName,"%s_orients.png",prefixTag);
    cvSaveImage(pbMapName, tmpOrient);    
  }
  cvReleaseImage(&tmpImg);
  cvReleaseImage(&tmpOrient);
  return 0;  
}
