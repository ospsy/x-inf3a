    		...
    		for(int y=-wr; y<=wr; y++){
      		for(int x=-wr; x <=+wr; x++){
				...
				if ((radius > wr) || ((x == 0) && (y == 0)) ){
	  				...
	  				double theta = atan2(y,x)-(i*binSize); // to orient the disc!
						...
						const int nb)
{
	...
	double beta2 = 0.3;
  	...
  	int w = 0.01*sqrt(edgeMap_float->width*edgeMap_float->width + edgeMap_float->height*edgeMap_float->height);
							...
							int numOri)
{
	...
	double beta1 = 10;   
	double beta2 = 0.3;
	int w = 0.015*sqrt(edgeMap->width*edgeMap->width + edgeMap->height*edgeMap->height);
